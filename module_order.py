...
from module_utilities import create_report, read_option_chain, read_spot_price , calculate_mtm
...
def select_ikey(strategy_dict, status_dict, spot_price, nifty_options_df,option_type):
    ...
    def ikey_ltp(nifty_options_df, strategy_dict, status_dict, option_type):
        ...
        filtered_options = nifty_options_df[nifty_options_df['option_type'] == option_type]
        
        if filtered_options.empty:
            print(f"No options found for type {option_type}")
            return None
        
        filtered_options.loc[:, 'ltp_diff'] = abs(filtered_options['ltp'] - preferred_ltp)
    ...
    def ikey_strike(nifty_options_df, strategy_dict, status_dict, option_type):
        ...
        filtered_options.loc[:, 'strike_diff'] = abs(filtered_options['strike_price'] - preferred_strike)
    ...
    if strategy_dict['ikey_criteria'] == 'ATM':
        ...
        print(f"Invalid ikey_criteria: {strategy_dict['ikey_criteria']}")
        return None
...
def initialize_status_dict(status_dict, zone_index, option_type, instrument_key):
    ...
    status_dict.update({
        ...
        'entry_transaction': 'B' if zone_index in [1, 5] else 'S',
        'exit_transaction': 'S' if zone_index in [1, 5] else 'B',
        'real_quantity': None,
        'entry_order_id': None,
        'exit_order_id': None,
        'entry_success': False,
...
def place_order(client, status_dict, instrument_key, strategy_dict, order_flag):
    ...
    try:
        ...
        if order_flag == 'ENTRY':
            transaction = status_dict.get('entry_transaction')
        elif order_flag == 'EXIT':
            transaction = status_dict.get('exit_transaction')
        else:
            print("Invalid order_flag")
            return False


        # Determine quantity based on the index
        if strategy_dict.get('index') == 'BANKNIFTY':
            quantity = str(float(strategy_dict.get('quantity', 0)) * 15)
        elif strategy_dict.get('index') == 'NIFTY':
            quantity = str(float(strategy_dict.get('quantity', 0)) * 25)
        ...
        print(f"Exception when placing order: {e}")
        if order_flag == 'ENTRY' :
            status_dict['entry_success'] = False
...
def update_exit_status(client, status_dict,instrument_key):
    ...
    if order_details['stCode'] == 200:
        data = order_details['data'][0]
        status_dict['exit_success'] = "False" if data['ordSt'] == "rejected" else "True"
        status_dict['exit_success'] = "True"
        status_dict['exit_ltp'] = ltp
        try :
            status_dict['exit_time'] = data['ordDtTm']
        except :
...
def execute_order(zone_index, option_type, indicators , strategy_dict,  client):
    ...
    # Assign Necessary Flags and Setup Status Dict 
    #------------------------------------------------------------------------------------------------------
    status_dict = {}
    ...
    if instrument_key is None:
        ...
        print(f"No suitable instrument found for option_type: {option_type}")
        return
    else :
